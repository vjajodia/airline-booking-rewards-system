...
class Booking {
    ...
    static async cancelReservation(booking_id) {
        const query = ' DELETE FROM seat_booking WHERE booking_id = $1 AND state = \'Paid\';';
        await pool.query(query, [booking_id]);
    }
    ...
    static async getCancelledBookings(booking_id) {
        const query = 'SELECT * FROM seat_booking NATURAL JOIN flight_schedule NATURAL JOIN route WHERE booking_id = $1 ORDER BY booking_id, schedule_id';
        let bookingsDetails = await pool.query(query, [booking_id]);
